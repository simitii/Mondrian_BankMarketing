...
ATT_NAMES = ["age","job","marital","education","default","housing","loan",
...
def read_data():
    ...
    data_file = open('data/bank.csv', 'rU')
    ii = 1
    for line in data_file:
        ...
        ltemp.append(str(ii))#ltemp.append(temp[SA_INDEX])
        ii += 1
        data.append(ltemp)
    # pickle numeric attributes and get NumRange
    for i in range(QI_num):
        if IS_CAT[i] is False:
            static_file = open('data/bank_' + ATT_NAMES[QI_INDEX[i]] + '_static.pickle', 'wb')
...
def read_pickle_file(att_name):
    ...
    """
    try:
        static_file = open('data/bank_' + att_name + '_static.pickle', 'rb')
